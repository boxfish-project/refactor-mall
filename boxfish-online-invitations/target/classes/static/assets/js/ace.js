	...
	function enableDemoAjax() {		
		...
		if(window.Pace) {
			window.paceOptions = {
				...
				//elements: {selectors: ['.content-content-area']}
			}
		}
		...
		var demo_ajax_options = {
			 ...
			 'default_url': 'content/index',//default hash
			 'content_url': function(hash) {
				...
				//for example in Ace HTML demo version we convert /ajax/index.html#content/gallery to > /ajax/content/gallery.html and load it
				if(path.match(/(\/ajax\/)(index\.html)?/))
					return path.replace(/(\/ajax\/)(index\.html)?/, '/ajax/content/'+hash.replace(/^page\//, '')+'.html') ;

				//for example in Ace PHP demo version we convert "ajax.php#content/dashboard" to "ajax.php?content=dashboard" and load it
		...
		//initiate ajax loading on this element( which is .content-content-area[data-ajax-content=true] in Ace's demo)
	...
	function scrollTopBtn() {
		...
		if(scroll_btn.length > 0) {
			...
			$(window).on('scroll.scroll_btn', function() {
				...
				if(scroll > parseInt(h / 4) || (scroll > 0 && body_sH >= h && h + scroll >= body_sH - 1)) {//|| for smaller pages, when reached end of content
					if(!is_visible) {
	...
	function smallDeviceDropdowns() {
	  ...
	  function adjustNavbarDropdown() {
		...
		if( $sub.css('position') == 'fixed' ) {
			...
			//so for example when in 'content-slider' content, fixed modal toggle buttons go above are dropdowns
...
(function($ , undefined) {
	...
	//And it has a problem when sidebar is fixed & scrollable and there is a CSS3 animation inside content content.
	...
	function Sidebar_Scroll(sidebar , settings) {
		...
		var initiate = function(on_page_load) {
			...
			*/
			...
			//only the first time, on content load
...
(function($ , undefined) {
 ...
 var switch_direction = function() {
	...
	$('.content-content-area[data-ajax-content=true]').on('ajaxscriptsloaded.rtl', function() {
	...
	function applyChanges(el) {
		...
		swap_classes('messagebar-item-left', 'messagebar-item-right');//for inbox content
